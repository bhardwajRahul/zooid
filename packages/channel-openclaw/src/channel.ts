...
function resolveAccount(
  ...
  const clientId =
    (effective?.clientId ?? '').trim() ||
    (process.env.ZOOID_CLIENT_ID ?? '').trim() ||
    undefined;
  const clientSecret =
    (effective?.clientSecret ?? '').trim() ||
    (process.env.ZOOID_CLIENT_SECRET ?? '').trim() ||
    undefined;
...
export const zooidPlugin = {
  ...
  config: {
    ...
    isConfigured: (account: ResolvedZooidAccount) =>
      Boolean(
        account.serverUrl &&
        (account.token || (account.clientId && account.clientSecret)),
      ),
    unconfiguredReason: (account: ResolvedZooidAccount) => {
      if (!account.serverUrl) return 'serverUrl not configured';
      if (!account.token && !(account.clientId && account.clientSecret))
        return 'token or clientId+clientSecret not configured';
    ...
    describeAccount: (account: ResolvedZooidAccount) => ({
      ...
      authMode: account.clientId ? 'oauth' : 'token',
    }),
  ...
  setup: {
    ...
    validateInput: (params: {
      ...
      if (params.input.useEnv && params.accountId !== DEFAULT_ACCOUNT_ID) {
        return 'ZOOID_TOKEN / ZOOID_CLIENT_ID can only be used for the default account.';
      }
      const hasToken = params.input.useEnv || params.input.token;
      const hasOAuth = params.input.clientId && params.input.clientSecret;
      if (!hasToken && !hasOAuth) {
        return 'Zooid requires a token or clientId+clientSecret (or --use-env).';
      }
