...
describe('zooidPlugin config', () => {
  ...
  it('marks account as not configured when token and oauth are missing', () => {
    const saved = process.env.ZOOID_TOKEN;
    const savedId = process.env.ZOOID_CLIENT_ID;
    const savedSecret = process.env.ZOOID_CLIENT_SECRET;
    delete process.env.ZOOID_TOKEN;
    delete process.env.ZOOID_CLIENT_ID;
    delete process.env.ZOOID_CLIENT_SECRET;
    try {
      const cfg = createCfg();
      cfg.channels!.zooid!.token = '';
      const account = zooidPlugin.config.resolveAccount(cfg);
      expect(zooidPlugin.config.isConfigured(account)).toBe(false);
      expect(zooidPlugin.config.unconfiguredReason(account)).toContain(
        'not configured',
      );
    } finally {
      if (saved !== undefined) process.env.ZOOID_TOKEN = saved;
      if (savedId !== undefined) process.env.ZOOID_CLIENT_ID = savedId;
      if (savedSecret !== undefined)
        process.env.ZOOID_CLIENT_SECRET = savedSecret;
    }
  });

  it('marks account as configured with clientId and clientSecret', () => {
    const saved = process.env.ZOOID_TOKEN;
    delete process.env.ZOOID_TOKEN;
    try {
      const cfg = createCfg();
      cfg.channels!.zooid!.token = '';
      cfg.channels!.zooid!.clientId = 'cid';
      cfg.channels!.zooid!.clientSecret = 'csec';
      const account = zooidPlugin.config.resolveAccount(cfg);
      expect(zooidPlugin.config.isConfigured(account)).toBe(true);
      expect(account.clientId).toBe('cid');
      expect(account.clientSecret).toBe('csec');
    } finally {
      if (saved !== undefined) process.env.ZOOID_TOKEN = saved;
    }
  });
...
describe('zooidPlugin setup', () => {
  ...
  it('validates that token or oauth is required without useEnv', () => {
    const err = zooidPlugin.setup.validateInput({
      accountId: 'default',
      input: { serverUrl: 'https://example.com' },
    });
    expect(err).toContain('token');
    expect(err).toContain('clientId');
  });

  it('accepts clientId + clientSecret as alternative to token', () => {
    const err = zooidPlugin.setup.validateInput({
      accountId: 'default',
      input: {
  ...
  it('applies oauth config for default account', () => {
    const cfg: OpenClawConfig = {};
    const result = zooidPlugin.setup.applyAccountConfig({
      cfg,
      accountId: 'default',
